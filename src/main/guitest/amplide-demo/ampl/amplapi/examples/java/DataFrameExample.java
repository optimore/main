...
// This example illustrate the usage of the DataFrame class
public class DataFrameExample {
	...
	public static void main(String[] args) {
		...
		df2.setColumn("FOOD", new String[] { "BEEF", "CHK", "FISH", "HAM",
				"MCH", "MTL", "SPG", "TUR" });
		...
		df2.addColumn("cost", new double[] { 3.19, 2.59, 2.29, 2.89, 1.89,
				1.99, 1.99, 2.49 });
		...
		String[] nutrWithMultiplicity = new String[df1.getColumn("NUTR").length
				* df2.getColumn("FOOD").length];
		String[] foodWithMultiplicity = new String[df1.getColumn("NUTR").length
				* df2.getColumn("FOOD").length];
		...
		int[] values = new int[] { 60, 8, 8, 40, 15, 70, 25, 60, 10, 20, 15,
		...
		try {
			...
			System.out.format("Objective function value: %f%n", ampl
					.getObjective("total_cost").value());
