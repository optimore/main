function status = LNSlistmain(dataParameters, tabuParameters, logfileParameters, resultParameters)
%% LNS
status = 0;
...
try
    ...
    ampl_res_name = 'src/main/ampl/LNSModelList.res';
    
    % Open and replace fileds in files:
    fin = fopen('LNSModelList.run','rt');
    fout = fopen('LNSModelList_clone.run','wt');
    while ~feof(fin)
        s = fgets(fin);
    ...
    % Run LNS
    system('module add cplex/12.5-fullampl; ampl < LNSModelList_clone.run  >/dev/null')
    
    delete(close_msgbox1);
    msgbox('Finished')
    
    % Create result:
    fid = fopen('LNSModelList.res','r');
