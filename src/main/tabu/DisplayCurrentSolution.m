function [] = DisplayCurrentSolution(data,figaxes,figdata)
...
for i=1:length(data.tasks)
    line([data.tasks(i,6) data.tasks(i,6)+data.tasks(i,5)], [data.tasks(i,4) data.tasks(i,4)], ...
        'Color',Color(1+mod(i,3),:), 'LineWidth', i);
end
% hold(figaxes, 'off')
% 
...
% for i=1:size(tasks.dependencies,1)
% 
%     task1_start = data.dependencies()
% 
% TimelineSolution{DependencyMatrix(i,2)}(DependencyMatrix(i,1),1);
